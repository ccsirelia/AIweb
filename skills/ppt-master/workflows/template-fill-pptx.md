...
| Page count | The fill plan supports 1–100 output slides. `check-plan` and `apply` both reject larger plans. |
...
| Chart edit boundary | A single classic plot is editable when every series uses `c:cat/c:val`. Date-axis and multi-level categories are accepted with a warning because replacement categories are flattened to one level. Stock, scatter, bubble, ChartEx/unknown frames, multi-plot/combo charts, missing-series charts, and non-`c:cat/c:val` data models are rejected. Use beautify / main pipeline to redraw unsupported charts, or leave the native chart untouched. |
...
`apply` appends a timestamp and a short unique token automatically. For example, `-o "<project_dir>/exports/demo.pptx"` writes `demo_YYYYMMDD_HHMMSS_<unique>.pptx`. A filename already ending with that complete suffix is left unchanged.
...
| Removes source custom shows | Prevents stale custom-show relationship IDs after slides are omitted, reordered, or repeated |
...
| Check | Expected |
|---|---|
| Output filename | Ends with `_YYYYMMDD_HHMMSS_<unique>.pptx` |
