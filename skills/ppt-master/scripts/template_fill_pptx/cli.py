...
def _timestamped_pptx_path(path: Path) -> Path:
    if path.suffix.lower() != ".pptx":
        return path
    if re.search(r"_\d{8}_\d{6}_[0-9a-f]{12}$", path.stem, re.I):
        return path
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    token = uuid.uuid4().hex[:12]
    if re.search(r"_\d{8}_\d{6}$", path.stem):
        return path.with_name(f"{path.stem}_{token}{path.suffix}")
    return path.with_name(f"{path.stem}_{timestamp}_{token}{path.suffix}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    apply.add_argument(
        ...
        required=True,
        help=(
            "Output PPTX path. A _YYYYMMDD_HHMMSS_<unique> suffix is appended "
