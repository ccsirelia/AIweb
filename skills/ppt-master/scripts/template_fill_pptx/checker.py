...
def check_plan(library: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(plan_slides, list) and len(plan_slides) > MAX_PLAN_SLIDES:
        return {
            ...
            "results": [
                {
                    "status": "ERROR",
                    "code": "plan_slide_limit_exceeded",
                    "slide_count": len(plan_slides),
                    "maximum": MAX_PLAN_SLIDES,
                    "message": (
                        f"plan contains {len(plan_slides)} slides; "
                        f"maximum is {MAX_PLAN_SLIDES}"
                    ),
                }
            ],
        }

    for slide_index, slide in enumerate(plan_slides, start=1):
