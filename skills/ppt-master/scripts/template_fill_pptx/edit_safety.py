...
_NON_GENERIC_CATEGORY_PLOTS = frozenset(
    {"bubbleChart", "scatterChart", "stockChart"}
)
...
def _chart_edit_capability(chart_root: ET.Element) -> dict[str, Any]:
    ...
    if plot_type == "stockChart":
        return _unsupported_chart_capability(
            "chart_edit_stock_unsupported",
            (
                "template-fill chart edits do not rewrite stock charts because "
                "their required high/low/close series structure must be preserved"
            ),
            plot_type=plot_type,
            plot_count=1,
            data_model="stock",
...
def _is_verified_category_capability(capability: Any) -> bool:
    ...
    return bool(
        ...
        and capability.get("plot_type") not in _NON_GENERIC_CATEGORY_PLOTS
    )
